...
import { AssetStackCreateDto, AssetStackResponseDto, AssetStackUpdateDto, mapStack } from 'src/dtos/asset-stack.dto';
...
@Injectable()
export class AssetStackService {
  ...
  async create(auth: AuthDto, dto: AssetStackCreateDto): Promise<AssetStackResponseDto> {
  ...
  async get(auth: AuthDto, id: string): Promise<AssetStackResponseDto> {
    await this.access.requirePermission(auth, Permission.STACK_READ, id);
    const stack = await this.findOrFail(id);
    return mapStack(stack, { auth });
  }

  async update(auth: AuthDto, id: string, dto: AssetStackUpdateDto): Promise<AssetStackResponseDto> {
